...
2. `src/app/routes/sensor_data.py` parses the request body for indoor `temperature` and `humidity`.
3. `src/app/services/reading_service.py` loads the active `locations` row through `src/app/services/location_service.py`.
4. `src/app/services/reading_service.py` asks `src/app/services/weather_service.py` for current outdoor weather.
5. `src/app/services/weather_service.py` returns cached weather when a location was fetched within the last five minutes, otherwise it calls `src/app/services/openweather.py`.
6. If the location cannot be resolved or OpenWeather fails, no database row is created.
7. When weather data is available, a `Reading` model instance is created with the active `location_id`, indoor sensor values, and outdoor weather values.
8. `ensure_schema()` calls `db.create_all()`.
9. The reading is added to the session and committed.
10. If the commit fails, the session is rolled back and the failure is logged.
...
`src/app/routes/sensor_data.py` parses request parameters. `src/app/services/reading_service.py` handles reading queries, while `src/app/services/location_service.py` handles saved location queries:
